...
"Social Events":["At","Entity","Event","Old","New","Evidence","Confidence"],"Social Contradictions":["At","Entity","Field","Stored","Observed","Evidence","Status"],"Social Uncertainty":["At","Subject","Unknown","Needed For","Status"],"Social Research Queue":["At","Question","Reason","Source","Priority","Status","Findings","Evidence"],"Social Content Lineage":["At","Content ID","Idea","Evidence","Conversations","Platform","Text"],"Social Positions":["At","Topic","Position","Evidence","Supersedes","Status"],"Social Human Corrections":["At","Entity","Field","Correct Value","Reason","Active"],"Social Decision Replay":["At","Decision ID","Platform","Action","Input JSON","Reason","Confidence","Policy","Voice","Model","Schema"],"Social Policy Registry":["At","Policy","Version","Voice","Model","Thresholds","Status"],"Social Health":["At","Schema","Relationships","Actions","Opportunities","Outbox","OAuth","Sheets","Notes"],"Social Usage":["At","OpenAI Calls","LinkedIn Calls","Sheets Calls","Estimated Cost","Notes"],"Social Backups":["At","Snapshot ID","Location","Status","Notes"],
"Social Entity Graph":["At","Entity ID","Platform","Handle","Type","Relation","Evidence"],"Social Knowledge":["Claim","Status","Source","Verified At","Expires At","Confidence"],"Social Outcomes":["At","Platform","Account","Content ID","Outcome","Value"]}
...
def ensure_tabs():
 b=_client()
 if not b:return
 existing={s.title:s for s in b.worksheets()}
 for name,headers in TABS.items():
  if name not in existing:
   w=b.add_worksheet(title=name,rows=1000,cols=max(14,len(headers)));w.append_row(headers,value_input_option="RAW")
  else:
 ...
 if not b.worksheet("Social Schema").get_all_records():b.worksheet("Social Schema").append_row(["2.2",now(),"Cross-platform Social OS governance schema"],value_input_option="RAW")
def append(tab,row):
 try:
