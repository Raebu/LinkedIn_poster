...
import social_os,semantic,policy
...
def ask(prompt):
 corrections='; '.join(str(x.get('Correct Value') or x.get('Instruction') or '') for x in social_os.human_corrections()[-20:])
...
def generate(s):
 topic=random.choice(TOPICS);recent=content_context(s);shared_recent=social_os.recent_texts();fmt,angle,format_reason=choose_plan(topic,recent)
 for _ in range(4):
  d=parse(ask(f"Create ONE LinkedIn-native original post. Topic: {topic}. Chosen angle: {angle}. Format: {fmt}. Format rule: {FORMATS[fmt]} Recent posts:\n{recent}\nDo not repeat their argument. Return JSON only: {{\"text\":\"...\",\"topic\":\"...\",\"why_worth_saying\":\"...\",\"visual_brief\":\"\",\"document_outline\":[]}}. Complex thinking, simple language. One coherent argument. For M&A/investment use only relevant lenses such as strategic rationale, build-versus-buy, diligence, valuation assumptions, technology debt, IP, talent, integration, incentives and synergy quality. Never claim personal transaction experience unless verified context supplies it. No invented current facts, statistics, clients, transactions, meetings, product use or personal anecdotes. Do not depend on current news unless verified context is supplied. Natural paragraphs. No generic hook, generic concluding question or corporate filler."))
  if not d:continue
  ...
  if not ok or duplicate(text,s.get("posts",[])) or semantic.duplicate(text,shared_recent,.72):continue
  ok,reason=social_os.gate_generated(text)
  if not ok:
   if reason=='research_required':social_os.research_required(text,reason,'LinkedIn original post')
   continue
  if fmt=="visual" and len(str(d.get("visual_brief","")))<=40:continue
  if fmt=="document" and not 5<=len(d.get("document_outline",[]))<=8:continue
  review=parse(ask(f"Review this proposed LinkedIn {fmt} post:\n{text}\nReturn JSON only {{\"pass\":true,\"reason\":\"\"}}. Pass only if every paragraph earns its place, it is specific, intellectually useful, natural, reputation-safe, factual without unsupported certainty, complex thinking in simple language, not generic corporate content, and not political persuasion."))
...
def main():
 ...
 if not DRY:
  if not social_os.allowed('publish'):print('HELD: Publishing Enabled is FALSE');return
  if fmt in {"visual","document"} and not MEDIA_LIVE:print(f"HELD_FOR_MEDIA_VALIDATION: {fmt} rendered successfully but live media publishing remains disabled")
  else:
   li=LinkedIn();urn,asset_urn=(li.create_image_post(text,asset_bytes,alt) if fmt=="visual" else li.create_document_post(text,asset_bytes,title=f"{topic.title()} — Martin Raeburn.pdf") if fmt=="document" else (li.create_post(text),""));published=True;print("PUBLISHED",urn,"ASSET",asset_urn);social_os.lineage(urn,topic,why,"",text,"LinkedIn");social_os.replay('publish',{'urn':urn,'topic':topic},'published after shared Social OS gates',.9)
 else:social_os.replay('post_candidate',{'topic':topic,'text':text},'passed shared Social OS gates',.8)
